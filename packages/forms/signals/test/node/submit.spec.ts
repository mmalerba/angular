...
describe('submit', () => {
  ...
  it('runs action on invalid form with validationMode: ignore', async () => {
    const data = signal({first: '', last: ''});
    const f = form(
      data,
      (name) => {
  ...
  it('waits for pending validation with validationMode: waitForPending', async () => {
    const data = signal('');
