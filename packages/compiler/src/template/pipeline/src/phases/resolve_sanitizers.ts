    ...
    for (const op of view.update) {
      switch (op.kind) {
        ...
        case ir.OpKind.InterpolateStyleMap:
          // The compiler does not fill in a sanitizer for style binding values because the
          // style algorithm knows internally what props are subject to sanitization (only style
          // values set via [attr.style] are explicitly sanitized)
          // TODO: is the context ever anything other than style for these props? if not we can just
          // remove this whole case block.
