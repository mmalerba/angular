  ...
  for (const op of view.ops()) {
    ...
    switch (op.kind) {
      case ir.OpKind.Attribute:
        extractAttributeOp(op, elements, compatibility);
        break;
...
function isStringLiteral(expr: o.Expression): expr is o.LiteralExpr&
    {value: string} {return expr instanceof o.LiteralExpr && typeof expr.value === 'string'}

function extractAttributeOp(
    op: ir.AttributeOp, elements: Map<ir.XrefId, ir.ElementOrContainerOps>,
    compatibility: boolean) {
  const isLiteralValue = op.value instanceof o.LiteralExpr && typeof op.value.value === 'string';
  ...
  if (op.name === 'style' && isStringLiteral(op.value)) {
    ...
    const parsedStyles = parseStyle(op.value.value);
    ...
    let extractable = compatibility ? isStringLiteral(op.value) : (op.value.isConstant());
